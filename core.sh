...
function htmx_page() {
  if [[ -z "$NO_STYLES" ]]; then
    if [[ -z "$TAILWIND" ]]; then
      STYLE_TEXT='<link rel="stylesheet" href="/static/style.css?v='${GIT_REVISION}'">'
    else
  ...
  <link rel="icon" href="/static/plotter.png" type="image/png" />
  ${STYLE_TEXT}
...
<script src="/static/main.js?v=${GIT_REVISION}"></script>
</body>
</html>
EOF
}
